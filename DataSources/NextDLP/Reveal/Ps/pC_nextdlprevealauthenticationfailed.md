...
```Java
{
Name = nextdlp-reveal-authentication-failed
  Vendor = NextDLP
  Product = Reveal
  Lms = Direct
  DataType = "authentication-failed"
  TimeFormat = "yyyy-MM-dd'T'HH:mm:ss.SSSSSSSSSZ"
  Conditions = [ """reveal""", """"tags": ["""", """"sensor_type": """", """name": """", """Login with unauthorized account""" ]
  Fields = [
    """"timestamp"{1,10}:\s{0,100}"{1,10}({time}\d\d\d\d-\d\d-\d\dT\d\d:\d\d:\d\d.\d{1,10}Z)""""
    """"agent_hostname":\s{0,100}"({dest_host}[\w\-\.]{1,2000})""""
