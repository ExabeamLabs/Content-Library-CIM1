...
```Java
{
Name = reveal-member-added
  Vendor = NextDLP
  DataType = "member-added"
  Conditions = [ """reveal""", """User added to local security-enabled group""", """"tags":""", """"initialaccess"""", """"sensor_type": "AGENT_POLICY"""", """name":""" ]
  Fields = ${QUSHRevealParserTemplates.json-qush-reveal.Fields} [
    """({activity}User added)"""
    """User with SID\s{1,20}({user_sid}S-[^\s]{1,2000})"""
    """added to local group\s{0,100}(\[|({group_domain}[^\\]{1,2000})[\\]{1,100}({group_name}[^\s]{1,2000}))"""
  ]
