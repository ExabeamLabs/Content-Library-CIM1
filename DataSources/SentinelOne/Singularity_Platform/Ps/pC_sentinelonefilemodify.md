...
Name = sentinelone-file-modify
  DataType = "file-write"
  Conditions = [ """dproc=Deep Visibility Endpoint""", """destinationServiceName =SentinelOne""", """fileModification {""" ]
  Fields = ${SentinelOneParserTemplates.sentinelone-activity.Fields} [
    """({event_name}fileModification)""",
    """path:\s{0,100}"{1,20}({file_path}({file_parent}[^"]{1,2000})[\\\/]({file_name}[^"]{1,2000}?(\.({file_ext}[^".]{1,2000}))?))"""",
  ]
