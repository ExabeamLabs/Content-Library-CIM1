...
Name = json-sentinelone-singularityp-alert-3
  ...
  Fields = [
    ...
    """"dst.ip.address\\{0,20}":\\{0,20}"({dest_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({dest_port}\d+))?"""
    """"src.ip.address\\{0,20}":\\{0,20}"({src_ip}((([0-9a-fA-F.]{1,4}):{1,2}){7}([0-9a-fA-F]){1,4})|(((25[0-5]|(2[0-4]|1\d|[0-9]|)\d)\.?\b){4}))(:({src_port}\d+))?"""
    ...
    """"event.network.connectionStatus\\{0,20}":\\{0,20}"({outcome}[^\\"]{1,2000})"""
  ]
