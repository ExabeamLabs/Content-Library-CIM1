...
```Java
{
Name = pan-ngfw-network-connection
  DataType = "network-connection"
  Conditions = [ """"LogType":"GTP"""", """"Action":""", """"Rule":""", """"DeviceName":""" ]
  Fields = ${PaloAltoParserTemplates.paloalto-vpn.Fields}[
    """"Action":"({action}[^"]{1,2000})"""",
    """"Rule":"({rule}[^"]{1,2000})"""",
    """"Application":"({app}[^"]{1,2000})"""",
    """"Subtype":"({outcome}[^"]{1,2000})"""",
    """"NATSource":"({src_translated_ip}[a-fA-F\d:.]{1,2000})""",
    """"NATDestination":"({dest_translated_ip}[a-fA-F\d:.]{1,2000})""",
    """"NATSourcePort":({src_translated_port}\d{1,5})""",
    """"NATDestinationPort":({dest_translated_port}\d{1,5})""",
    """"Bytes":({bytes}\d{1,10}),""",
    """"BytesSent":({bytes_out}\d{1,10}),""",
    """"BytesReceived":({bytes_in}\d{1,10}),"""
  ]
