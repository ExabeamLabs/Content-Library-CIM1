...
```Java
{
Name = pan-wildfire-alert-1
  DataType = "alert"
  Conditions = [ """"LogType":"THREAT"""", """"Subtype":"wildfire"""" ]
  Fields = ${PaloAltoParserTemplates.paloalto-vpn.Fields}[
    """"Action":"({action}[^"]{1,20000})"""",
    """"VendorSeverity":"({alert_severity}[^"]{1,2000})"""",
    """"ThreatCategory":"((?i:unknown)|({threat_category}[^"]{1,2000}))"""",
    """"Subtype":"({alert_type}[^"]{1,2000})"""",
    """"ThreatID":"({alert_name}[^"\(]{1,20000}?)\s{0,100}(\(({alert_id}\d{1,100})\))?""""
  ]
